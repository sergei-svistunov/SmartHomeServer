...
std::ostream& operator<<(std::ostream& os, Device device) {
	unsigned int ids[16] = { 13, 5, 3, 11, 15, 7, 1, 9, 14, 6, 4, 12, 16, 8, 2,
			10 };
...
std::ostream& operator<<(std::ostream& os, Command command) {
	const char* commands[16] = { "ALL_UNITS_OFF", "ALL_LIGHTS_ON", "ON", "OFF",
			"DIM", "BRIGHT", "ALL_LIGHTS_OFF", "EXTENDED", "HAIL_REQUEST",
			"HAIL_ACKNOWLEDGE", "EXT3", "EXT4", "EXT2", "STATUS_ON",
...
bool Controller::SetAddr(Home home, Device device, uint8_t repeats) {
	uint8_t data[2] = { _GetHeader(repeats, HeaderType::ADDRESS, false),
			(uint8_t) ((uint8_t) home << 4 | (uint8_t) device) };
...
bool Controller::SendCommand(Home home, Command command, uint8_t repeats) {
	uint8_t data[2] = { _GetHeader(repeats, HeaderType::FUNCTION, false),
			(uint8_t) ((uint8_t) home << 4 | (uint8_t) command) };
...
void Controller::_RecieveData() {
	...
	vector<unsigned short int> tmp(buffer,
			buffer + sizeof(buffer) / sizeof(unsigned short int));
